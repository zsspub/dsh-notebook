...
import { IMAGE_MEDIA_TYPES } from "../types.js";
export const NOTEBOOK_SCHEMA_VERSION = 1;
const MEDIA_TYPES = new Set(IMAGE_MEDIA_TYPES);
...
function normalizedTags(values) {
    if (values === undefined)
        return [];
    if (values.length > 50)
        throw new NotebookError('tags must contain at most 50 entries');
    const tags = new Set();
...
function positiveInteger(name, value) {
    if (!Number.isSafeInteger(value) || value <= 0)
        throw new NotebookError(`${name} must be a positive integer`);
    return value;
}
function nonNegativeInteger(name, value) {
    if (!Number.isSafeInteger(value) || value < 0)
        throw new NotebookError(`${name} must be a non-negative integer`);
    return value;
}
function iso(value) {
...
function assertAbsolute(name, value) {
    if (value !== ':memory:' && !isAbsolute(value))
        throw new NotebookError(`${name} must be absolute or :memory:`);
}
...
function parseJsonArray(value, label) {
    ...
    if (!Array.isArray(parsed))
        throw new NotebookError(`invalid ${label} snapshot`);
    return parsed;
}
...
export class NotebookStore {
    ...
    constructor(config, dependencies = {}) {
        ...
        if (config.defaultPageSize > config.maxPageSize)
            throw new NotebookError('defaultPageSize cannot exceed maxPageSize');
    ...
    recoverTrash() {
        ...
        for (const revision of revisions) {
            for (const image of parseJsonArray(revision.images_json, 'image'))
                live.add(image.storageName);
        }
    ...
    ensureOpen() {
        if (this.closed)
            throw new NotebookError('notebook store is closed');
    }
    notebookRow(id) {
        const row = this.database.prepare('SELECT * FROM notebooks WHERE id = ?').get(id);
        if (!row)
            throw new NotebookError(`notebook ${id} was not found`, 'NOT_FOUND');
        return row;
    }
    noteRow(id) {
        const row = this.database.prepare('SELECT * FROM notes WHERE id = ?').get(id);
        if (!row)
            throw new NotebookError(`note ${id} was not found`, 'NOT_FOUND');
        return row;
    }
    tags(noteId) {
        return this.database.prepare('SELECT tag_name FROM note_tags WHERE note_id = ? ORDER BY tag_name').all(noteId)
    ...
    resolveNotebook(input, timestamp) {
        ...
        if (input.notebookId !== undefined)
            return this.notebookRow(input.notebookId);
    ...
    async prepareImage(noteId, input, position) {
        if (!MEDIA_TYPES.has(input.mediaType))
            throw new NotebookError(`unsupported image type ${input.mediaType}`);
        let data;
        try {
    ...
    `);
        for (const image of images) {
            statement.run(image.id, image.noteId, image.name, image.mediaType, image.bytes, image.width, image.height, image.digest, image.storageName, image.position, image.createdAt);
        }
    }
    cleanupPublished(images) {
        for (const image of images)
            rmSync(join(this.config.imageDirectory, image.storageName), { force: true });
    }
    ...
    createNotebook(nameInput) {
        ...
        if (!notebook)
            throw new NotebookError('created notebook is unavailable');
        return notebook;
    }
    ...
    async create(input) {
        ...
        if (uploads.length > this.config.maxImagesPerNote)
            throw new NotebookError('too many images for one note');
    ...
    async update(input) {
        ...
        if (removedIds.size !== currentImages.length - retained.length)
            throw new NotebookError('removeImageIds contains an unknown image');
        const uploads = input.images ?? [];
        if (retained.length + uploads.length > this.config.maxImagesPerNote)
            throw new NotebookError('too many images for one note');
        ...
        try {
            this.transaction(() => {
                ...
                const notebook = input.notebookId === undefined && input.notebookName === undefined
                    ? this.notebookRow(current.notebook_id)
    ...
    setArchived(input, archivedAt) {
        this.transaction(() => {
            ...
            if ((note.archived_at !== null) === (archivedAt !== null))
                throw new NotebookError('note already has the requested archive state');
    ...
    restoreRevision(input) {
        this.transaction(() => {
            ...
            const snapshot = this.database.prepare('SELECT * FROM note_revisions WHERE note_id = ? AND revision = ?')
                .get(input.id, input.revision);
            if (!snapshot)
                throw new NotebookError(`revision ${String(input.revision)} was not found`, 'NOT_FOUND');
      ...
      `);
            for (const image of images) {
                insert.run(image.id, input.id, image.name, image.mediaType, image.bytes, image.width, image.height, image.digest, image.storageName, image.position, image.createdAt);
            }
            this.refreshSearch(input.id);
            this.recordRevision(input.id);
        });
    ...
    deleteNotebook(input) {
        this.transaction(() => {
            ...
            if (count > 0)
                throw new NotebookError('non-empty notebooks cannot be deleted');
        ...
        try {
            this.transaction(() => {
                ...
                if (note.archived_at === null)
                    throw new NotebookError('archive a note before permanent deletion');
            ...
            for (const file of staged) {
                if (existsSync(file.trashPath))
                    renameSync(file.trashPath, join(this.config.imageDirectory, file.storageName));
            }
            throw error;
        }
        for (const file of staged)
            rmSync(file.trashPath, { force: true });
        return { ok: true };
    }
    ...
    imageData(input) {
        ...
        if (!row)
            throw new NotebookError(`image ${input.id} was not found`, 'NOT_FOUND');
