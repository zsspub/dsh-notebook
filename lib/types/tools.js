...
function currentHumanImages(exec) {
    ...
    for (let index = events.length - 1; index >= 0; index -= 1) {
        ...
        return (message.content ?? [])
            .filter((part) => part.type === 'image' && part.attachment !== undefined)
...
export function apply(ctx) {
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            const note = await ctx.notebook.archive({ id: args.id, expectedRevision: args.expectedRevision }, exec.signal);
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            const note = await ctx.notebook.restore({ id: args.id, expectedRevision: args.expectedRevision }, exec.signal);
