...
const workspace = join(root, 'node_modules', '.cache', 'dsh-notebook-typert')
const packageRoot = join(workspace, 'packages', 'dsh-notebook')
...
manifest.exports = { '.': manifest.exports['.'], './types': manifest.exports['./types'] }
...
await writeFile(join(workspace, 'tsconfig.host.json'), JSON.stringify({
  ...
  references: [{ path: './packages/typert-protocol' }, { path: './packages/dsh-notebook' }],
}, null, 2))
await writeFile(join(protocolRoot, 'package.json'), JSON.stringify({
  name: '@deepseek-ai/dsh-typert-protocol', type: 'module', exports: { '.': './lib/index.js' },
}, null, 2))
...
export abstract class TypertRemoteService { protected constructor(_ctx: Context, _serviceKey: string) {} }
...
await writeFile(join(packageRoot, 'tsconfig.json'), JSON.stringify({
  compilerOptions: {
    target: 'ES2024', module: 'NodeNext', moduleResolution: 'NodeNext', strict: true, skipLibCheck: true,
    allowImportingTsExtensions: true, rewriteRelativeImportExtensions: true, noEmit: true, types: ['node'],
    baseUrl: '.', paths: {
...
try {
  const model = new WorkspaceAnalyzer({ root: workspace, packages: ['dsh-notebook'], faces: ['host'] }).analyze()
  const face = model.faces.find(candidate => candidate.face === 'host')
  const packageModel = face?.packages.find(candidate => candidate.name === 'dsh-notebook')
  if (!face || !packageModel) throw new Error('Typert found no Host package model')
  const artifact = new FaceModelEmitter(face).emit('dsh-notebook')
  if (!artifact.remote) throw new Error(`Typert found ${String(packageModel.invocations.length)} Remote methods`)
